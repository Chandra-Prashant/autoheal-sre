...
def run_bug(bug: dict) -> dict:
    repo_path = seed(bug)
    trace = capture_trace(repo_path, bug["test_target"])
    context = build_context(repo_path, trace)

    state = AgentState(trace=trace, repo_path=repo_path, context=context)
...
def _write_results(results: list[dict]) -> tuple[float, float]:
    pass_at_1 = sum(r["pass_at_1"] for r in results) / len(results)
    pass_at_3 = sum(r["pass_at_3"] for r in results) / len(results)
    with open(RESULTS_PATH, "w") as f:
        json.dump({"pass_at_1": pass_at_1, "pass_at_3": pass_at_3, "results": results}, f, indent=2)
...
def main():
    ...
    pass_at_1, pass_at_3 = _write_results(results)
    print(f"\npass@1: {pass_at_1:.2f}  pass@3: {pass_at_3:.2f}")
